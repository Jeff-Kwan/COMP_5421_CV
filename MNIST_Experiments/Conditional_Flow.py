...
def train_flow_matching(model, dataloader, num_epochs=5, lr=1e-3, wd=1e-2):
    ...
    for epoch in range(num_epochs):
        ...
        for images, classes in tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}", unit="batch"):
            images = images.to(device, non_blocking=True)  # MNIST images: [batch, 1, 28,28]
    ...
    torch.save(model.state_dict(), "MNIST_Experiments/Output/MNIST_flow_model.pth")
...
def plot_digit_samples(model, steps=20, fixed_sample=None):
    ...
    plt.tight_layout()
    plt.savefig(f"MNIST_Experiments/Output/sample_digits-{steps}-steps.png")
...
if __name__ == '__main__':
    os.makedirs("MNIST_Experiments/Output", exist_ok=True)
    train_model()